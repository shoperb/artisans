...
module Artisans
  module SettingsColorLexer
    ...
    # REGULAR_EXPRESSIONS[:settings_color] = settings_re(HEXCOLOR)
    ...
    def settings_color
      return unless @scanner.match?(REGULAR_EXPRESSIONS[:settings_color])
      return unless @scanner[2].length == 4 || @scanner[2].length == 7
      scanned_color = scan(REGULAR_EXPRESSIONS[:settings_color])
      script_color = Script::Value::Color.from_hex(@scanner[2])
...
Sass::Script::Lexer.send(:include, Artisans::SettingsColorLexer)
